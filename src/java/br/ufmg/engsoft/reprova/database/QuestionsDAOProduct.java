...
public class QuestionsDAOProduct {
	private final MongoCollection<Document> collection;

	public QuestionsDAOProduct(MongoCollection<Document> getCollection) {
	...
	*/
	public Question get(String id) {
		...
		Question question = (Question) this.collection.find(eq(new ObjectId(id))).map(this::parseDoc).first();
		if (question == null)
			QuestionsDAO.getLogger().info(String.format("No such question %s", id));
		return question;
	}
	...
	*/
	public boolean remove(String id) {
		...
		if (result)
			QuestionsDAO.getLogger().info(String.format("Deleted question %s", id));
		else
			QuestionsDAO.getLogger().warn(String.format("Failed to delete question %s", id));
		return result;
	}
	...
	*/
	public boolean add(Question question) {
		...
		if (id != null) {
			...
			if (!result.wasAcknowledged()) {
				QuestionsDAO.getLogger().warn(String.format("Failed to replace question %s", id));
				return false;
			}
		} else
	...
	private <U> U parseDoc(Document document1) {
		return null;
	}
}